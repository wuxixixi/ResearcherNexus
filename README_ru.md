...
ResearcherNexus- это фреймворк для глубокого исследования, разработанный сообществом и основанный на впечатляющей работе сообщества открытого кода. Наша цель - объединить языковые модели со специализированными инструментами для таких задач, как веб-поиск, сканирование и выполнение кода Python, одновременно возвращая пользу сообществу, которое сделало это возможным.

Пожалуйста, посетите [наш официальный сайт](https://ResearcherNexus.tech/) для получения дополнительной информации.
...
В этой демонстрации мы показываем, как использовать ResearcherNexus для:
...
- [Какова высота Эйфелевой башни по сравнению с самым высоким зданием?](https://ResearcherNexus.tech/chat?replay=eiffel-tower-vs-tallest-building)
- [Какие репозитории самые популярные на GitHub?](https://ResearcherNexus.tech/chat?replay=github-top-trending-repo)
- [Написать статью о традиционных блюдах Нанкина](https://ResearcherNexus.tech/chat?replay=nanjing-traditional-dishes)
- [Как украсить съемную квартиру?](https://ResearcherNexus.tech/chat?replay=rental-apartment-decoration)
- [Посетите наш официальный сайт, чтобы изучить больше повторов.](https://ResearcherNexus.tech/#case-studies)
...
ResearcherNexus разработан на Python и поставляется с веб-интерфейсом, написанным на Node.js. Для обеспечения плавного процесса настройки мы рекомендуем использовать следующие инструменты:
...
ResearcherNexus поддерживает несколько поисковых систем, которые можно настроить в файле `.env` с помощью переменной `SEARCH_API`:
...
ResearcherNexus теперь включает функцию преобразования текста в речь (TTS), которая позволяет конвертировать исследовательские отчеты в речь. Эта функция использует API TTS volcengine для генерации высококачественного аудио из текста. Также можно настраивать такие параметры, как скорость, громкость и тон.
...
ResearcherNexus использует LangGraph для своей архитектуры рабочего процесса. Вы можете использовать LangGraph Studio для отладки и визуализации рабочего процесса в реальном времени.

#### Запуск LangGraph Studio локально

ResearcherNexus включает конфигурационный файл `langgraph.json`, который определяет структуру графа и зависимости для LangGraph Studio. Этот файл указывает на графы рабочего процесса, определенные в проекте, и автоматически загружает переменные окружения из файла `.env`.
...
ResearcherNexus поддерживает трассировку LangSmith, чтобы помочь вам отладить и контролировать ваши рабочие процессы. Чтобы включить трассировку LangSmith:
...
```bash
# Замените ResearcherNexus-api-app на предпочитаемое вами имя контейнера
...
ResearcherNexus предоставляет настройку docker-compose для легкого запуска бэкенда и фронтенда вместе:
...
Следующие примеры демонстрируют возможности ResearcherNexus:
...
ResearcherNexus включает механизм "человек в контуре", который позволяет вам просматривать, редактировать и утверждать планы исследования перед их выполнением:
...
ResearcherNexus создан на основе невероятной работы сообщества открытого кода. Мы глубоко благодарны всем проектам и контрибьюторам, чьи усилия сделали ResearcherNexus возможным. Поистине, мы стоим на плечах гигантов.
...
- **[LangGraph](https://github.com/langchain-ai/langgraph)**: Их инновационный подход к оркестровке многоагентных систем сыграл решающую роль в обеспечении сложных рабочих процессов ResearcherNexus.
...
Сердечная благодарность основным авторам `ResearcherNexus`, чье видение, страсть и преданность делу вдохнули жизнь в этот проект:

- **[Daniel Walnut](https://github.com/hetaoBackend/)**
- **[Henry Li](https://github.com/magiccube/)**

Ваша непоколебимая приверженность и опыт стали движущей силой успеха ResearcherNexus. Мы считаем за честь иметь вас во главе этого путешествия.
