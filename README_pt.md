...
Nesse demo, nós demonstramos como usar o ResearcherNexus para:
...
- [Quão alta é a Torre Eiffel comparada ao prédio mais alto?](https://ResearcherNexus.tech/chat?replay=eiffel-tower-vs-tallest-building)
- [Quais são os top repositórios tendência no GitHub?](https://ResearcherNexus.tech/chat?replay=github-top-trending-repo)
- [Escreva um artigo sobre os pratos tradicionais de Nanjing's](https://ResearcherNexus.tech/chat?replay=nanjing-traditional-dishes)
- [Como decorar um apartamento alugado?](https://ResearcherNexus.tech/chat?replay=rental-apartment-decoration)
- [Visite nosso site oficial para explorar mais replays.](https://ResearcherNexus.tech/#case-studies)
...
ResearcherNexus é desenvolvido em Python, e vem com uma IU web escrita em Node.js. Para garantir um processo de configuração fácil, nós recomendamos o uso das seguintes ferramentas:
...
ResearcherNexus suporta múltiplos mecanismos de busca que podem ser configurados no seu arquivo `.env` usando a variável `SEARCH_API`:
...
ResearcherNexus agora inclui uma funcionalidade Texto-para-Fala (TTS) que permite que você converta relatórios de busca para voz. Essa funcionalidade usa o mecanismo de voz da API TTS para gerar áudio de alta qualidade a partir do texto. Funcionalidades como velocidade, volume e tom também são customizáveis.
...
ResearcherNexus usa LangGraph para sua arquitetura de fluxo de trabalho. Nós podemos usar o LangGraph Studio para debugar e visualizar o fluxo de trabalho em tempo real.

#### Rodando o LangGraph Studio Localmente

ResearcherNexus inclui um arquivo de configuração `langgraph.json` que define a estrutura do grafo e dependências para o LangGraph Studio. Esse arquivo aponta para o grafo do fluxo de trabalho definido no projeto e automaticamente carrega as variáveis de ambiente do arquivo `.env`.
...
```bash
# substitua ResearcherNexus-api-app com seu nome de container preferido
...
ResearcherNexus fornece uma estrutura docker-compose para facilmente executar ambos o backend e frontend juntos:
...
Os seguintes exemplos demonstram as capacidades do ResearcherNexus:
...
ResearcherNexus inclue um mecanismo de humano no processo que permite a você revisar, editar e aprovar planos de pesquisa antes que estes sejam executados:
...
ResearcherNexus é construído através do incrível trabalho da comunidade open-source. Nós somos profundamente gratos a todos os projetos e contribuidores cujos esforços tornaram o ResearcherNexus possível. Realmente, nós estamos apoiados nos ombros de gigantes.
...
- **[LangGraph](https://github.com/langchain-ai/langgraph)**: A abordagem inovativa para orquestração multi-agente deles tem sido foi fundamental em permitir o acesso dos fluxos de trabalho sofisticados do ResearcherNexus.
...
Um sincero muito obrigado vai para os principais autores do `ResearcherNexus`, cuja visão, paixão, e dedicação trouxe esse projeto à vida:

- **[Daniel Walnut](https://github.com/hetaoBackend/)**
- **[Henry Li](https://github.com/magiccube/)**

O seu compromisso inabalável e experiência tem sido a força por trás do sucesso do ResearcherNexus. Nós estamos honrados em tê-los no comando dessa trajetória.

## Histórico-Estrelas

[![Gráfico do Histórico de Estrelas](https://api.star-history.com/svg?repos=bytedance/ResearcherNexus&type=Date)](https://star-history.com/#bytedance/ResearcherNexus&Date)
